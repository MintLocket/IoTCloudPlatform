...
public class App implements RequestHandler<Document, String> {
    ...
    private String DYNAMODB_TABLE_NAME = "KioskSensor";
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        if (document.current.state.reported.light.equals(
                document.previous.state.reported.light) &&
                document.current.state.reported.sound.equals(
                        document.previous.state.reported.sound)) {
            return null;
        }

        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item()
                        .withPrimaryKey("kioskId", document.device)
                        .withLong("time", document.timestamp)
                        .withString("light", document.current.state.reported.light)
                        .withString("sound", document.current.state.reported.sound)
